...
class FFmpegManager:
    ...
    def _check_availability(self) -> bool:
        ...
        # Check local installation first - require both ffmpeg and ffprobe
        local_available = os.path.exists(self.ffmpeg_exe) and os.path.exists(self.ffprobe_exe)
        if local_available:
            return True
        # Check system PATH
    ...
    def _get_missing_files(self) -> List[str]:
        ...
        missing_files = []
        file_mapping = {
            "ffmpeg-6.0-essentials_build/bin/ffmpeg.exe": self.ffmpeg_exe,
            "ffmpeg-6.0-essentials_build/bin/ffprobe.exe": self.ffprobe_exe,
            "ffmpeg-6.0-essentials_build/bin/ffplay.exe": self.ffplay_exe,
            "ffmpeg-6.0-essentials_build/LICENSE": self.license_file
        }

        for zip_path, local_path in file_mapping.items():
