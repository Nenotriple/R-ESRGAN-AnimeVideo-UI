...
def _handle_progress_update(downloaded: int, total_size: int, progress_callback: Optional[Callable[[int, int], None]]) -> None:
    """Handle progress updates using callback or default printer."""
...
def _download(url: str, output_path: str, chunk_size: int, progress_callback: Optional[Callable[[int, int], None]] = None) -> bool:
    ...
    try:
        with urllib.request.urlopen(url) as response, open(output_path, 'wb') as out_file:
            ...
            while True:
                ...
                _handle_progress_update(downloaded, total_size, progress_callback)
        return True
...
def download_file(url: str, output_path: str, chunk_size: int = 8192, progress_callback: Optional[Callable[[int, int], None]] = None) -> bool:
    ...
    return _download(url, output_path, chunk_size, progress_callback)
